function [ output_args ] = plotSurface(tr1,Schnittpunkte,Schnittpunktebene,k,varargin)
%plots the surface
%  plots the surface :)
...
patch([-xlim -xlim xlim xlim -sx*xlim -sx*xlim -xlim],[ylim -ylim -ylim -sy*ylim -sy*ylim ylim ylim],zeros(1,7),'FaceColor','k','FaceAlpha',.5);
line([Schnittpunkte(1:(k-1),:,1);Schnittpunktebene(:,1)'],[Schnittpunkte(1:(k-1),:,2);Schnittpunktebene(:,2)'],[Schnittpunkte(1:(k-1),:,3);Schnittpunktebene(:,3)'],'Color','k','LineWidth',0.3);
scatter3(Schnittpunktebene(:,1)',Schnittpunktebene(:,2)',zeros(1,np),'*r');
scatter3(sx*safetyFactor*Schnittpunkte(2,:,1)',sy*safetyFactor*Schnittpunkte(2,:,2)',zeros(1,np),'.g');
line([Schnittpunktebene(:,1)';sx*safetyFactor*Schnittpunkte(2,:,1)], [Schnittpunktebene(:,2)';sy*safetyFactor*Schnittpunkte(2,:,2)], zeros(2,np),'Color','r');
title(sprintf('fresnel construction; rays on contact grid: %d/%d',numOnContactGrid,np));
xlabel('x');
ylabel('y');
zlabel('z');
%axis equal;
