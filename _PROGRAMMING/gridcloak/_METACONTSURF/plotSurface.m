function [ ] = plotSurface(orig,Schnittpunkt,Schnittpunktebene,x_orig,y_orig,z,varargin)
%plots the surface
%  plots the surface :)
...
numOnContactGrid=numOnContactGrid+sum(isnan(Schnittpunktebene(:,1)));
%% plot
...
%plot distances
figure;
dist = reshape(sqrt((Schnittpunktebene(:,1)'-safetyFactor*sx*Schnittpunkt(:,1)').^2+(Schnittpunktebene(:,2)'-safetyFactor*sy*Schnittpunkt(:,2)').^2),[],1);
pcolor(x_orig,y_orig,reshape(dist,size(x_orig,1),size(x_orig,2)))
caxis([0,80]);
title(sprintf('distance from design point, average: %2.1f\n# rays on contact grid: %d/%d',mean(sqrt((Schnittpunktebene(:,1)'-safetyFactor*sx*Schnittpunkt(:,1)').^2+(Schnittpunktebene(:,2)'-safetyFactor*sy*Schnittpunkt(:,2)').^2),'omitnan'),numOnContactGrid,np));
colormap jet;
xlabel('x');
ylabel('y');
c=colorbar;
c.Label.String = 'average distance from designpoint';
shading interp;
